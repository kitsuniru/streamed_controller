...
typedef StateConsumerListener<S> = void Function(BuildContext context, S previous, S current);
...
typedef StateConsumerBuilder<S> = Widget Function(BuildContext context, S state, Widget? child);
...
class StateConsumer<S extends Object> extends StatefulWidget {
  ...
  final StreamedController<S> controller;
...
class _StateConsumerState<S extends Object> extends State<StateConsumer<S>> {
  late StreamedController<S> _controller;
  ...
  void didUpdateWidget(StateConsumer<S> oldWidget) {
    ...
    final oldController = oldWidget.controller, newController = widget.controller;
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) => super.debugFillProperties(properties
    ..add(DiagnosticsProperty<StreamedController<S>>('Controller', _controller))
    ..add(DiagnosticsProperty<S>('State', _controller.state))
    ..add(FlagProperty('isProcessing', value: _controller.isProcessing, ifTrue: 'Processing', ifFalse: 'Idle')));
